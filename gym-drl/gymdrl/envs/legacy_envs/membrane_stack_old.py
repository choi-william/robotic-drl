...
OBJ1_POS = [BOX_WIDTH*(OBJ_SIZE/2+0.06), BOX_WIDTH*(OBJ_SIZE/2 + LINK_HEIGHT/2)]
OBJ2_POS = [BOX_WIDTH*(OBJ_SIZE/2+0.06), BOX_WIDTH*(OBJ_SIZE*3/2 + LINK_HEIGHT/2)]
TARGET_POS = OBJ2_POS
GRAVITY = -30
...
class MembraneStack(gym.Env):
    ...
    def _reset(self):
        ...
        object1_position = (BOX_WIDTH*(OBJ_SIZE/2+0.06), BOX_WIDTH*(OBJ_SIZE/2 + LINK_HEIGHT/2))
        object2_position = (BOX_WIDTH*(1-OBJ_SIZE/2-0.06), BOX_WIDTH*(OBJ_SIZE/2 + LINK_HEIGHT/2))
...
class MembraneWithoutLinkages(MembraneStack):
    with_linkage = False
