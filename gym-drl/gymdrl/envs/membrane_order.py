...
#   - places the green block before the yellow block, horizontally aligned
# 
# Copyright (c) 2017 William Choi, Alex Kyriazis, Ivan Zinin; all rights reserved

FPS = 50
# Set the object size for the squares here
OBJ_SIZE = membrane_base.BOX_WIDTH*0.1
...
class MembraneOrder(gym.Env):
    ...
    def _reset(self):
        ...
        object1_position = (membrane_base.BOX_WIDTH/5, OBJ_SIZE/2 + membrane_base.LINK_HEIGHT/2)
        object2_position = (membrane_base.BOX_WIDTH*4/5, OBJ_SIZE/2 + membrane_base.LINK_HEIGHT/2)
    ...
    def _step(self, action):
        ...
        obj_dist_x = object2_pos[0] - object1_pos[0] + OBJ_SIZE
        obj_dist_y = object2_pos[1] - object1_pos[1]

        shaping = -200*np.abs(obj_dist_x)/membrane_base.BOX_WIDTH -100*np.abs(obj_dist_y)/membrane_base.BOX_HEIGHT -30*max(actuator_pos)/membrane_base.BOX_HEIGHT
